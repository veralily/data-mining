...
def main(_):
  ...
  vectors_data = np.vstack((vectors_data2,vectors_data3))
  print(vectors_data.shape)
  labels_data = np.vstack((np.reshape(labels_data2,(len(labels_data2),1)),np.reshape(labels_data3,(len(labels_data3),1))))
  ...
  tlabels_data = tlabels_data[0:1000,]
  ...
  with tf.Graph().as_default(), tf.Session() as session:
    ...
    for i in range(config.max_max_epoch):
      ...
      train_perplexity = run_epoch(session, m, vectors_data, labels_data, m.train_op,
                                   verbose=True)
      print("Epoch: %d Train Perplexity: %.3f" % (i + 1, train_perplexity))

      valid_perplexity = run_epoch(session, mvalid, validation_data, vlabels_data, tf.no_op())
      print("Epoch: %d Valid Perplexity: %.3f" % (i + 1, valid_perplexity))

    test_perplexity = run_epoch(session, mtest, test_data, tlabels_data, tf.no_op())
